...
export default function ExamTakingPage({ paperId }) {
    ...
    if (questions.length === 0) {
        return (
            <div className="min-h-screen flex flex-col items-center justify-center bg-slate-50 dark:bg-slate-900 text-slate-800 dark:text-white">
                ...
                <button onClick={() => router.visit('/student/exams')} className="px-4 py-2 bg-blue-600 text-white rounded-lg">Go Back</button>
            </div>
        );
    }
    ...
    return (
        <div className="min-h-screen bg-slate-50 dark:bg-slate-900 flex flex-col text-slate-900 dark:text-white transition-colors">
            <Head title="Taking Exam" />
            
            {/* Header */}
            <header className="bg-white dark:bg-slate-800 border-b border-slate-200 dark:border-slate-700 h-16 flex items-center justify-between px-6 sticky top-0 z-10 shrink-0">
                <div className="font-bold text-lg hidden sm:block">
                    Question {currentIndex + 1} of {questions.length}
                </div>
                
                {remainingTime ? (
                    <div className={`flex items-center font-bold px-4 py-1.5 rounded-full ${remainingTime.minutes < 5 && remainingTime.hours === 0 ? 'bg-red-100 text-red-600 dark:bg-red-900/30 dark:text-red-400' : 'bg-slate-100 text-slate-700 dark:bg-slate-700 dark:text-slate-200'}`}>
                ...
                <button 
                    ...
                    className="font-bold text-blue-600 hover:text-blue-700 dark:text-blue-400 dark:hover:text-blue-300 disabled:opacity-50 transition-colors"
                >
                    Finish
                </button>
            </header>
            ...
            <main className="flex-1 overflow-y-auto p-4 sm:p-8 flex justify-center">
                <div className="w-full max-w-3xl">
                    <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-xl shadow-slate-200/50 dark:shadow-none border border-slate-200 dark:border-slate-700 p-6 sm:p-10">
                        ...
                        <div className="mb-10">
                            ...
                            {question.image_url && (
                                <img src={question.image_url} alt="Question" className="mt-6 max-h-64 rounded-xl border border-slate-200 dark:border-slate-700" />
                            )}
                        </div>
                            ...
                            {['A', 'B', 'C', 'D'].map(optLabel => {
                                ...
                                return (
                                    <button
                                        ...
                                        className={`w-full text-left p-4 sm:p-5 rounded-xl border-2 transition-all flex items-center group ${
                                            isSelected 
                                    ...
                                    >
                                        <div className={`w-10 h-10 rounded-full flex items-center justify-center font-bold text-lg mr-4 transition-colors ${
                                            isSelected
                                            ? 'bg-blue-600 text-white dark:bg-blue-500'
                                            : 'bg-slate-100 text-slate-600 dark:bg-slate-700 dark:text-slate-300 group-hover:bg-blue-100 dark:group-hover:bg-blue-900/50'
                                        }`}>
                                            {optLabel}
                                        </div>
                        ...
                        <div className="flex items-center justify-between mt-12 pt-8 border-t border-slate-100 dark:border-slate-700">
                            <button
                                ...
                                className="px-6 py-3 rounded-xl border border-slate-200 dark:border-slate-700 font-medium text-slate-600 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-700 disabled:opacity-50 disabled:cursor-not-allowed flex items-center transition-colors"
                            >
                            ...
                            {isLastQuestion ? (
                                ...
                                <button
                                    onClick={() => setCurrentIndex(prev => prev + 1)}
                                    className="px-8 py-3 rounded-xl bg-blue-600 hover:bg-blue-700 text-white font-bold flex items-center transition-colors"
                                >
                                    Next
