                                    ...
                                    <div key={q.id} className="border border-slate-200 dark:border-slate-700 rounded-lg p-5 hover:border-blue-300 dark:hover:border-blue-700 transition-colors">
                                        ...
                                        <div className="grid grid-cols-1 md:grid-cols-2 gap-3 ml-7">
                                            {options.map((optText, i) => {
                                                ...
                                                return (
                                                    <div key={i} className={`p-2 rounded-lg text-sm border ${isCorrect ? 'bg-green-50 border-green-200 dark:bg-green-900/20 dark:border-green-800 text-green-800 dark:text-green-300 font-medium' : 'bg-slate-50 border-slate-200 dark:bg-slate-800 dark:border-slate-700 text-slate-600 dark:text-slate-400'}`}>
            ...
            <Modal isOpen={isImportModalOpen} onClose={() => setIsImportModalOpen(false)} title="Import JSON Questions" maxWidth="2xl">
                <div className="space-y-4">
                    <div className="flex justify-between items-center mb-2">
                        <span className="text-sm font-medium text-slate-700 dark:text-slate-300">Paste your JSON array here:</span>
                        <button onClick={copyJsonFormat} className="flex items-center text-sm text-blue-600 hover:text-blue-700 font-medium">
                            <Copy className="w-4 h-4 mr-1" /> Copy Format
                        </button>
                    </div>
                    <textarea
                        className="w-full px-4 py-3 bg-slate-50 dark:bg-slate-900/80 border border-slate-200 dark:border-slate-700 rounded-lg font-mono text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 text-slate-900 dark:text-white"
                        rows="15"
                        placeholder={`[
  {
    "question": "Sample?",
