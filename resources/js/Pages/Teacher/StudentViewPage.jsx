...
export default function StudentViewPage({ studentId }) {
    ...
    if (!student) {
        return (
            <TeacherLayout title="Student Details">
                <div className="text-center py-24 text-slate-500">Student not found.</div>
            </TeacherLayout>
        );
    }
    ...
    return (
        <TeacherLayout title={`Student: ${student.name}`}>
            ...
            <div className="mb-6">
                <Link href="/teacher/students" className="inline-flex items-center text-sm font-medium text-slate-600 dark:text-slate-400 hover:text-blue-600 dark:hover:text-blue-400 transition-colors">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div className="lg:col-span-1">
                    <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-700 p-6 text-center transition-colors">
                        <div className="w-32 h-32 mx-auto rounded-full bg-blue-100 dark:bg-blue-900/50 flex items-center justify-center text-blue-700 dark:text-blue-400 text-4xl font-bold mb-4 border-4 border-white dark:border-slate-800 shadow-md">
                            {student.name.charAt(0).toUpperCase()}
                        </div>
                        <h2 className="text-2xl font-bold text-slate-900 dark:text-white mb-1">{student.name}</h2>
                        <p className="text-slate-500 dark:text-slate-400 text-sm mb-6">Enrolled {new Date(student.enrollment_date || Date.now()).toLocaleDateString()}</p>
                        
                        <div className="space-y-3 text-left">
                            <div className="flex items-center p-3 rounded-lg bg-slate-50 dark:bg-slate-900/50">
                                <Mail className="w-5 h-5 text-slate-400 mr-3" />
                                <span className="text-slate-700 dark:text-slate-300">{student.email}</span>
                            </div>
                            <div className="flex items-center p-3 rounded-lg bg-slate-50 dark:bg-slate-900/50">
                                <Phone className="w-5 h-5 text-slate-400 mr-3" />
                                <span className="text-slate-700 dark:text-slate-300">{student.phone || 'N/A'}</span>
                            </div>
                        </div>
                    </div>
                </div>
                    ...
                    <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-700 p-6 transition-colors">
                        ...
                        <div className="space-y-4">
                            {student.courses && student.courses.length > 0 ? student.courses.map(course => (
                                <div key={course.id} className="border border-slate-100 dark:border-slate-700 p-4 rounded-xl">
                            ...
                            <table className="w-full text-left text-sm">
                                ...
                                <tbody>
                                    {student.recent_exams && student.recent_exams.length > 0 ? student.recent_exams.map(exam => {
                                        ...
                                        return (
                                            <tr key={exam.id} className="border-b border-slate-100 dark:border-slate-700/50 last:border-0">
                                                <td className="py-4 font-medium text-slate-900 dark:text-white">
                                                    <Link href={`/teacher/exams/${exam.id}/student/${student.id}`} className="hover:text-blue-600 dark:hover:text-blue-400 transition-colors">
                                                        {exam.title}
                                                    </Link>
                                                </td>
                                                ...
                                                <td className="py-4 text-right font-bold text-slate-900 dark:text-white">
                                                    <span className={`px-2.5 py-1 rounded-lg text-sm ${percentage >= 75 ? 'bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400' : percentage >= 50 ? 'bg-yellow-100 text-yellow-700 dark:bg-yellow-900/30 dark:text-yellow-400' : 'bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-400'}`}>
                                        ...
                                        <tr>
                                            <td colSpan="3" className="py-4 text-center text-slate-500 dark:text-slate-400">No recent exams found.</td>
                                        </tr>
                                    )}
                                </tbody>
                            </table>
