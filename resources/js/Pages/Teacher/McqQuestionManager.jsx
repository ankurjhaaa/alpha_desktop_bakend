...
export default function McqQuestionManager({ paperId }) {
    ...
    return (
        <TeacherLayout title={`Questions for Paper #${paperId}`}>
            ...
            <div className="mb-4">
                <Link href="/teacher/mcq-papers" className="inline-flex items-center text-sm font-medium text-slate-600 dark:text-slate-400 hover:text-blue-600 dark:hover:text-blue-400 transition-colors">
                    ...
                    {isLoading ? (
                        ...
                        <div className="space-y-4">
                            {questions.map((q, index) => (
                                <div key={q.id} className="border border-slate-200 dark:border-slate-700 rounded-lg p-5 hover:border-blue-300 dark:hover:border-blue-700 transition-colors">
                                    <div className="flex justify-between items-start mb-4">
                                        <div className="flex-1">
                                            <div className="flex items-start">
                                                <span className="font-bold text-blue-600 dark:text-blue-400 mr-2">Q{index + 1}.</span>
                                                <h3 className="text-slate-900 dark:text-white font-medium">{q.question_text}</h3>
                                            </div>
                                            {q.image_url && (
                                                <div className="mt-3 ml-7">
                                                    <img src={q.image_url} alt="Question" className="max-h-32 rounded-lg border border-slate-200 dark:border-slate-700" />
                                                </div>
                                            )}
                                        </div>
                                        <div className="flex items-center space-x-2 ml-4">
                                            <span className="px-2 py-1 bg-slate-100 dark:bg-slate-700 text-slate-600 dark:text-slate-300 text-xs rounded font-medium">
                                                {q.marks} Mark{q.marks !== 1 && 's'}
                                            </span>
                                            <button onClick={() => openModal(q)} className="p-1.5 text-blue-600 hover:bg-blue-50 dark:hover:bg-blue-900/30 rounded transition-colors" title="Edit Question">
                                                <Edit2 className="w-4 h-4" />
                                            </button>
                                            <button onClick={() => handleDelete(q.id)} className="p-1.5 text-red-600 hover:bg-red-50 dark:hover:bg-red-900/30 rounded transition-colors" title="Delete Question">
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3 ml-7">
                                        {['A', 'B', 'C', 'D'].map(opt => {
                                            ...
                                            return (
                                                <div key={opt} className={`p-2 rounded-lg text-sm border ${isCorrect ? 'bg-green-50 border-green-200 dark:bg-green-900/20 dark:border-green-800 text-green-800 dark:text-green-300 font-medium' : 'bg-slate-50 border-slate-200 dark:bg-slate-800 dark:border-slate-700 text-slate-600 dark:text-slate-400'}`}>
                                    ...
                                    {q.explanation && (
                                        <div className="mt-4 ml-7 p-3 bg-blue-50 dark:bg-blue-900/10 border border-blue-100 dark:border-blue-900/30 rounded-lg text-sm text-slate-700 dark:text-slate-300">
            ...
            <Modal isOpen={isModalOpen} onClose={closeModal} title={editingQuestion ? "Edit Question" : "Add New Question"} maxWidth="4xl">
                <form onSubmit={handleSave} className="space-y-5">
                    ...
                    <div className="flex items-center space-x-4">
                        <div className="flex-1">
                            <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1.5">Question Image (Optional)</label>
                            <input
                                type="file"
                                accept="image/*"
                                onChange={handleImageChange}
                                className="w-full px-3 py-1.5 text-sm bg-slate-50 dark:bg-slate-900/50 border border-slate-200 dark:border-slate-700 rounded-lg file:mr-4 file:py-1.5 file:px-3 file:rounded-md file:border-0 file:text-sm file:font-semibold file:bg-blue-50 file:text-blue-700 hover:file:bg-blue-100 dark:file:bg-blue-900/30 dark:file:text-blue-400"
                            />
                        </div>
                        {imagePreview && (
                            <div className="relative w-24 h-24 rounded border border-slate-200 dark:border-slate-700 overflow-hidden bg-slate-100 dark:bg-slate-800 flex items-center justify-center">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1.5">Correct Option</label>
                            <select
